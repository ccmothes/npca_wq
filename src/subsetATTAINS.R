subsetATTAINS <- function(layer, type, path = 'data/'){
  ...
  nps_attains <- nps_attains %>%
    sf::st_intersection(., dplyr::select(nps_all_catchments, nhdplusid = FEATUREID, UNIT_CODE))
  ...
  saveRDS(nps_attains, paste0(path, '/00_nps_attains_', type, '.RDS'))
  sf::st_write(dplyr::select(nps_attains, UNIT_CODE, assessmentunitidentifier), paste0(path, '/00_nps_attains_', type, '.shp'), append = FALSE)
