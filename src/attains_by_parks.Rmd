...
# Add a line
points(x, y2, pch=18, col="blue", type="b", lty=2)
# Add a legend
legend(1, 15, legend=c("Impaired Waters"),
        pt.bg = c("#DC851E"),  pch = c(21,21), cex = 1.3)#, lty=1:2, cex=1.4, border = "none", )
legend(1, 60, legend=c("Tier 3", "Tier 3 and Tier 2.5", "Tier 2.5"),
       pt.bg = c( "#059FA4","#DC851E", "#A1A522"),  pch = c(21,21), cex = 1.3)#, lty=1:2, cex=1.4, border = "none", )
...
usa <- tigris::states(resolution = "5m") %>% filter(!NAME %in% c("Michigan", "Hawaii")) %>%
  st_transform(3857) %>%
  bind_rows(prettier)
rm(prettier)
...
count <- nps_all_attains %>% filter(ircategory %in% c("5A","5","4C","4B", "4A")) %>%
...
impaired_parks <- nps_all_attains %>%
  ...
  distinct(UNIT_CODE, .keep_all =TRUE)
# list303d <- nps_all_attains %>%
#   dplyr::filter(parametercategorycode %in% c("5A","5")) %>% 
#   distinct(UNIT_CODE, .keep_all =TRUE)
# 
...
# list303d_mapper <- parks %>%
#   dplyr::filter(UNIT_CODE %in% list303d$UNIT_CODE) %>%
#   sf::st_centroid() %>%
#   st_join(st_transform(tigris::states(), 3857)) %>%
#   usa_mapper()
...
ggplot() +
  ...
  #geom_sf(data = list303d_mapper, fill = "#DC851E", color = "black", size = 3, pch = 21) 
