pseudoATTAINSsubset <- function(layer, type, path = 'data/'){
    
    # find all ATTAINS features within NPS boundaries
    nps_attains <- layer %>%
      #.[parks,] %>%
      sf::st_join(., dplyr::select(parks, UNIT_CODE), left = FALSE)
    ...
    nps_attains <- nps_attains %>%
      sf::st_intersection(., dplyr::select(attains_catchments, nhdplusid))
    ...
    saveRDS(nps_attains, paste0(path, '/00_nps_pseudo_attains_', type, '.RDS'))
    try(sf::st_write(dplyr::select(nps_attains, UNIT_CODE, assessmentunitidentifier), paste0(path, '/00_nps_pseudo_attains_', type, '.shp'), append = FALSE))
