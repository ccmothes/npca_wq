usa_mapper<-  function (input_sf){
  ...
  # Alaska
    if (any(ak_check)) {
      ...
      #                                                        0.07 * (bb$ymax - bb$ymin)), scale = 0.5, 
  ...
  # Virgin Islands
    if (any(vi_check)) {
      vi_rescaled <- sf::st_transform(us_vi, 
        ...
        sf::st_geometry(vi_rescaled) <- place_geometry_wilke(sf::st_geometry(vi_rescaled), 
                                                             c(bb$xmin + 1.06 * (bb$xmax - bb$xmin), bb$ymin - 
                                                                 0.00 * (bb$ymax - bb$ymin)), scale = 2, 
