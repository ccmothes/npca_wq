catchmentizeATTAINS <- function(layer, path = 'data/') {
  ...
  nps_attains_to_catchment <- nps_all_catchments %>%
    dplyr::select(nhdplusid = FEATUREID, UNIT_CODE) %>%
    as_tibble(.) %>%
  ...
  saveRDS(nps_attains_to_catchment, paste0(path, '/00_nps_attains_to_catchment.RDS'))
  sf::st_write(select(nps_attains_to_catchment, UNIT_CODE, assessmentunitidentifier, nhdplusid), paste0(path, '/00_nps_attains_to_catchment.shp'), append = FALSE)
